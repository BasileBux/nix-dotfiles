...
let
  ...
  # Playwright's chromium-headless-shell lives in a revision-suffixed dir with a
  # platform-specific binary name (on linux-arm64: chrome-linux/headless_shell).
  # We resolve the executable generically at build time (installPhase) AND set
  # PLAYWRIGHT_BROWSERS_PATH. `browsers` is hoisted here so the installPhase can
  # reference it via ${browsers}.
...
stdenvNoCC.mkDerivation (finalAttrs: {
  ...
  installPhase = ''
    ...
    # The playwright browser lives under a revision-suffixed dir with a
    # platform-specific binary name (linux-arm64 uses chrome-linux/headless_shell
    # here). Resolve it to ONE concrete executable at build time, so the shell
    # never globs the `*` into multiple args (which would desync makeWrapper).
    playwright_chrome=$(find -L "${browsers}"/chromium_headless_shell-* \( -name headless_shell -o -name chrome-headless-shell \) -type f 2>/dev/null | head -1 || true)
    ...
    makeWrapper $out/bin/start.sh $out/bin/linkwarden \
      ...
      --set-default PLAYWRIGHT_LAUNCH_OPTIONS_EXECUTABLE_PATH "$playwright_chrome" \
